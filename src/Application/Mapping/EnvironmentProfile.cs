...
public class EnvironmentProfile : Profile
{
    public EnvironmentProfile()
    {
        CreateMap<Environment, EnvironmentDto>()
            .ForMember(
                dest => dest.Services,
                opt => opt.MapFrom(src => src.EnvironmentServices.Select(es => es.Service))
            )
            .ForMember(
                dest => dest.EnvironmentAreas,
                opt => opt.MapFrom(src => src.EnvironmentAreas)
            )
            ...
            .ForMember(
                dest => dest.SpecialAvailabilities,
                opt => opt.MapFrom(src => src.SpecialAvailabilities)
            )
            .ForMember(
                dest => dest.DiscountPolicies,
                opt => opt.MapFrom(src => src.DiscountPolicies)
            )
        ...
        CreateMap<CreateEnvironmentDto, Environment>()
            ...
            .ForMember(
                dest => dest.WeeklySchedules,
                opt => opt.MapFrom(src => src.WeeklySchedules)
            );
        ...
        CreateMap<Reservation, ReservationResponse>()
            .ForMember(
                dest => dest.EnvironmentTitle,
                opt => opt.MapFrom(src => src.Environment.Title)
            )
            .ForMember(
                dest => dest.EnvironmentPublicId,
                opt => opt.MapFrom(src => src.Environment.PublicId)
            )
            .ForMember(dest => dest.OwnerId, opt => opt.MapFrom(src => src.Environment.OwnerId))
            .ForMember(
                dest => dest.RentalUnit,
                opt => opt.MapFrom(src => src.Environment.RentalUnit)
            )
            .ForMember(
                dest => dest.EnvironmentPhotoUrl,
                opt =>
                    opt.MapFrom(src =>
                        src.Environment.Photos.OrderBy(p => p.Order)
                            .Select(p => p.Url)
                            .FirstOrDefault()
                    )
            )
